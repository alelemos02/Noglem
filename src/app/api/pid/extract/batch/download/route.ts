...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/pid/extract/batch/download`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.detail || "Erro ao gerar Excel consolidado" },
    ...
    return new NextResponse(blob, {
      headers: {
        ...
        "Content-Disposition": response.headers.get("content-disposition") || "attachment; filename=instrument_index_consolidado.xlsx",
      },
    });
  } catch (error) {

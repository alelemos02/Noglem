...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/pid/extract/download`, {
      method: "POST",
    ...
    return new NextResponse(blob, {
      headers: {
        ...
        "Content-Disposition": response.headers.get("content-disposition") || "attachment; filename=instrument_index.xlsx",
      },
    });
  } catch (error) {

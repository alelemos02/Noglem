...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/civil/processar`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.detail || "Erro ao processar o PDF" },
        { status: response.status }
      );
    }

    const blob = await response.blob();
    const contentDisposition = response.headers.get("content-disposition") || "attachment; filename=quantitativo.xlsx";

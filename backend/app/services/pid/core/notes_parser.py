...
def parse_notes(
    ...
    Args:
        ...
        scale: DocumentScale for this page (scales pixel tolerances).
...
def _find_notes_region(
    ...
    x_margin = page_width * 0.3  # Allow 30% width range (page-relative, always scale-safe)
    top_margin = _s(scale, 5.0)  # Tolerance above header
...
def _parse_numbered_notes(lines: List[str]) -> List[DrawingNote]:
    ...
    for line in lines:
        ...
        if match:
            ...
            # Bug fix: wrap int() in try/except to handle malformed note numbers
            try:

...
def parse_title_block(
    ...
    metadata.sheet_number, metadata.total_sheets = _extract_sheet_info(tb_words, tb_text)
...
def _extract_document_number(words: List[ExtractedWord], text: str) -> str:
    patterns = [
...
def _extract_title(
    ...
    line_tol = _s(scale, 10.0)
    title_tol = _s(scale, 15.0)

    for w in words:
        upper = w.text.upper()
        if any(kw in upper for kw in title_keywords):
            y_ref = w.position.top
            nearby = [ww for ww in words if abs(ww.position.top - y_ref) < line_tol]
...
def _extract_sheet_info(words: List[ExtractedWord], text: str) -> tuple:
    patterns = [

...
def parse_title_block(
    ...
    metadata.title = _extract_title(tb_words, tb_text)
...
def _extract_title(words: List[ExtractedWord], text: str) -> str:
    ...
    for i, w in enumerate(words):
        ...
        if any(kw in upper for kw in title_keywords):
            ...
            nearby = [
                ww for ww in words
                if abs(ww.position.top - y_ref) < 10
            ]
    ...
    for i, w in enumerate(words):
        if w.text.upper() in ("TITLE", "TÍTULO", "TITULO"):
            ...
            nearby = [
                ww for ww in words
                if abs(ww.position.top - y_ref) < 15 and ww.position.x0 > w.position.x1
            ]
            if nearby:
                nearby.sort(key=lambda ww: ww.position.x0)
